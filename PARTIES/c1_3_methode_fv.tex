...
La diversité des méthodes restantes ne sera pas abordée en profondeur. La méthode retenue pour ce travail est la méthode de Monte Carlo Ray Tracing qui est reconnue pour sa généricité. Deux autres méthodes sont néanmoins présentées dans la suite de cette section, afin de comprendre comment sont établis les facteurs de vue pour des géométries simples.
...
L'intégrale obtenue est ensuite traitée analytiquement par introduction d'une fonction auxiliaire dépendant d'un paramètre, puis par décomposition du terme de pondération en contributions élémentaires. Cette démarche conduit à une expression analytique du facteur de vue :

\begin{equation}
F_{16}=\frac{2}{\pi} \left[ \ln\left(\frac{2}{\sqrt{3}}\right) +2\sqrt{2}\arctan\left(\frac{1}{\sqrt{2}}\right)-\frac{\pi}{2} \right] \simeq 0.1998.
\end{equation}
