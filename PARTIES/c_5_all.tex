...
La sensibilité de la solution thermique à la discrétisation axiale est étudiée pour les trois niveaux de puissance considérés précédemment. Le paramètre $N_z$ désigne le nombre de sous-géométries issues de la décomposition du domaine selon l'axe $z$. Il ne s'agit pas d'un raffinement du maillage au sens numérique du terme, mais d'une discrétisation spatiale de la représentation des propriétés thermiques et radiatives : à chaque sous-géométrie est associée une température uniforme, qui sert de température effective dans le calcul des échanges radiatifs.
...
Pour chaque niveau de puissance, les températures minimale, maximale et moyenne sont comparées à la solution de référence obtenue pour $N_z=20$. L'écart relatif est défini par
\begin{equation}
\varepsilon_T(N_z)
=
...
Avant d'analyser les résultats, il est utile de préciser l'origine physique de l'erreur introduite par cette représentation. Le flux surfacique émis par rayonnement obéit à la loi de Stefan--Boltzmann,
\begin{equation}
q_{\mathrm{rad}} = \varepsilon \sigma T^4,
\label{eq:stefan}
\end{equation}
où $\varepsilon$ est l'émissivité de la surface et $\sigma$ la constante de Stefan--Boltzmann. Le caractère convexe de la fonction $T \mapsto T^4$ implique, d'après l'inégalité de Jensen, que pour toute distribution de température non uniforme sur une surface $A$,
\begin{equation}
\overline{T^4}
=
\frac{1}{A}\int_A T^4\,\mathrm{d}A
\;>\;
\overline{T}^{\,4},
\qquad
\overline{T} = \frac{1}{A}\int_A T\,\mathrm{d}A,
\label{eq:jensen}
\end{equation}
l'égalité n'étant atteinte que pour un champ uniforme. La représentation d'une sous-géométrie par une température uniforme sous-estime donc systématiquement la puissance radiative émise dès qu'un gradient thermique est présent, et cette sous-estimation croît avec l'amplitude des variations de température au sein de la sous-géométrie. Cette inégalité constitue le fondement physique de l'ensemble des écarts discutés dans cette section.
...
La Figure~\ref{fig:convergence3MW} met en évidence une convergence monotone des trois grandeurs vers la solution de référence lorsque $N_z$ augmente : l'accroissement du nombre de sous-géométries permet de résoudre de plus en plus finement les variations thermiques axiales. À partir de $N_z=14$, les écarts relatifs sont de l'ordre de $10^{-3}$, ce qui indique que le gain apporté par un raffinement supplémentaire devient marginal au regard des autres approximations du modèle.

Cette convergence s'interprète directement à la lumière de l'inégalité~\eqref{eq:jensen}. Dans chaque sous-géométrie, les propriétés radiatives sont évaluées à partir d'une température de surface uniforme. Une discrétisation axiale grossière assimile donc une portion étendue du profil thermique à une température unique, ce qui sous-estime la puissance rayonnée. Lorsque $N_z$ augmente, l'étendue axiale représentée par chaque température diminue, l'écart $\overline{T^4}-\overline{T}^{\,4}$ se réduit, et le bilan radiatif calculé tend vers celui du champ continu. Le raffinement axial ne constitue donc pas une simple amélioration géométrique : il détermine la fidélité avec laquelle le champ de température est « vu » par le modèle radiatif.
...
\begin{table}[H]
\centering
\caption{Écarts relatifs maximaux observés par rapport à la référence $N_z=20$.}
...
Pour ce niveau de puissance, un raffinement jusqu'à $N_z=14$ suffit ainsi à obtenir une représentation axiale convergée. Au-delà, la poursuite du raffinement devient peu pertinente, l'erreur de discrétisation étant déjà faible devant les autres sources d'incertitude du modèle : le choix de $N_z$ résulte d'un compromis entre la précision du bilan radiatif et le coût de calcul.
...
Comme pour le niveau de puissance précédent, on observe une convergence vers la solution de référence, sans modification de la structure thermique globale : le crayon central demeure le plus chaud et les crayons d'angle les plus froids. La discrétisation axiale ne modifie là encore que les niveaux locaux de température.

La hiérarchie des écarts est également conservée, mais leur amplitude augmente légèrement : $T_{\max}$ est surestimée jusqu'à $0{,}9\,\%$ (contre $0{,}7\,\%$ précédemment), tandis que $T_{\min}$ et $\overline{T}$ sont sous-estimées jusqu'à $4\,\%$ et $0{,}5\,\%$ respectivement.
...
Cette amplification est cohérente avec la physique du transfert radiatif. D'une part, la dépendance en $T^4$ de la loi de Stefan--Boltzmann~\eqref{eq:stefan} confère au flux radiatif une sensibilité relative à la température quatre fois supérieure à celle de la température elle-même, puisque $\mathrm{d}q_{\mathrm{rad}}/q_{\mathrm{rad}} = 4\,\mathrm{d}T/T$ : à erreur relative de représentation constante, l'erreur sur le bilan radiatif croît avec le niveau de température. D'autre part, l'augmentation de la puissance accentue les gradients thermiques au sein des sous-géométries, ce qui élargit l'écart de l'inégalité~\eqref{eq:jensen} et dégrade d'autant la représentativité d'une température homogénéisée. Comme précédemment, l'erreur se concentre sur les températures extrêmes, tandis que la température moyenne bénéficie de compensations spatiales.

La vérification de la convergence du solveur conduit à la même conclusion que pour le cas précédent : avec une température moyenne stabilisée à $0{,}1~\mathrm{K}$ près, l'incertitude relative associée est de l'ordre de
\begin{equation}
\frac{0{,}1}{981{,}4}
\approx 1{,}0\times10^{-4},
\end{equation}
...
\begin{table}[H]
\centering
\caption{Écarts relatifs maximaux observés par rapport à la référence $N_z=20$ pour le niveau de puissance le plus élevé.}
...
L'écart spectaculaire sur $T_{\min}$ s'interprète dans ce cadre. La température d'une région froide est fixée par le bilan entre la puissance radiative reçue des surfaces chaudes environnantes, ses propres pertes et les transferts conductifs. La sous-estimation systématique des émissions des surfaces chaudes — conséquence directe de l'inégalité~\eqref{eq:jensen} appliquée à des sous-géométries fortement hétérogènes — fausse donc l'ensemble des flux incidents sur la région froide. L'erreur se propage ainsi à travers le réseau des échanges radiatifs et y est amplifiée par le fait que les zones froides échangent simultanément avec plusieurs surfaces voisines : les erreurs commises sur chacune d'elles se cumulent dans le bilan local.

Cette propagation explique également que la température moyenne soit désormais affectée de manière non négligeable. Un écart de $5\,\%$ ne peut plus relever d'une perturbation locale s'annulant par compensation spatiale : il traduit une modification du bilan radiatif suffisamment ample pour déplacer l'équilibre thermique global de l'assemblage.

La température maximale reste, à l'inverse, comparativement moins sensible, son écart ne passant que de $0{,}9\,\%$ à $3\,\%$. Cette asymétrie entre $T_{\min}$ et $T_{\max}$ montre que l'erreur de discrétisation ne dépend pas seulement du niveau absolu de température, mais aussi de la position de la région considérée dans le réseau d'échanges radiatifs : les régions froides, réceptrices nettes du rayonnement de nombreuses surfaces, cumulent les erreurs de représentation de leur environnement, tandis que le point chaud n'est affecté que par l'erreur commise sur sa propre sous-géométrie.
...
Cette analyse confirme que le raffinement axial est un paramètre déterminant pour la représentativité du calcul radiatif. Aux niveaux de puissance les plus élevés, une discrétisation suffisamment fine est indispensable pour limiter l'erreur associée à l'homogénéisation de la température par sous-géométrie et garantir une précision acceptable sur l'ensemble du champ thermique, en particulier dans les régions les plus froides.
...
\end{equation}
avec $A$, $C$ et $\tau$ des paramètres ajustés sur les données. Cette expression traduit la saturation progressive de l'écart relatif aux températures élevées, où le transfert radiatif tend à homogénéiser le champ thermique. La faisabilité d'une telle démarche sur un assemblage 5$\times$5, bien que simplifié par rapport à une géométrie de cœur réel, démontre le potentiel de cette approche pour des configurations industrielles plus représentatives.
